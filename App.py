...
with col1:
    ...
    uploaded_1d = st.file_uploader("Upload Daily Chart", type=["png", "jpg", "jpeg"], key="up_1d")
    ...
    if uploaded_1d:
        ...
        st.write("📥 Please upload a Daily chart image to start.")
...
with col2:
    ...
    uploaded_1h = st.file_uploader("Upload 1H Chart", type=["png", "jpg", "jpeg"], key="up_1h")
    ...
    if uploaded_1h:
        ...
        if high_l and low_l:
            ...
            cv2.line(open_cv_image, (0, high_l), (w, high_l), (255, 0, 0), 4) # Blue line for Buy-side Liquidity
            cv2.line(open_cv_image, (0, low_l), (w, low_l), (0, 255, 0), 4) # Green line for Sell-side Liquidity
            
        st.image(open_cv_image, use_container_width=True, caption="Detected Liquidity Levels (Blue/Green)")
        st.success("Successfully identified Buy-side Liquidity (BSL) and Sell-side Liquidity (SSL).")
    else:
        st.write("📥 Please upload a 1-Hour chart image.")
...
with col3:
    ...
    try:
        ...
        st.info("💡 Price is currently trading near key technical zones.")
    except Exception as e:
        st.error(f"Error fetching POI data: {e}")
...
with col4:
    ...
    try:
        ...
        take_profit = entry_price * 1.01 # 1.0% TP
        ...
        st.error(f"Error fetching 1M timeframe data: {e}")
